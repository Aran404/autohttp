...
## Cross-language contracts (Go <-> Python)
- Prefer explicit contracts over inferred behavior. If Go and Python exchange data, define the contract once and generate both sides from it.
- Use Protocol Buffers/gRPC for service contracts when applicable. For REST/JSON, use OpenAPI as the source of truth and generate clients or types for both Go and Python.
- Version external contracts explicitly, such as `v1/` and `v2/`, and avoid changing wire fields on only one side.
- Keep shared contract definitions in one obvious location and update both language bindings in the same change.
- CI must build and typecheck both sides against shared contracts after contract changes.
